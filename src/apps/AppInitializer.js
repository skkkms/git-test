...
class AppInitializer {
  constructor() {
    ...
    Vue.config.errorHandler = function (err, vm, info) {
      console.log('에러 로그 API 호출 해야 함');
      console.log('errorHandler', err);
      console.log('errorHandler', vm);
  ...
  initPageLoader() {
    ...
    window.utils.install('openWindowPopup', (pageId, params = {}, title = pageId, options = {}) => {
      ...
      if (pageId) {
        url = `${_.replace(systemConfig.publicPath, /^\/$/, '')}${systemConfig.popupUrl}#/${pageId}${queryString}`;
  ...
  init() {
    ...
    window.rules.setI18n(this.i18n)
  }
  ...
  createVueApp(VueApp, loginData) {
    ...
    document.title = utils.getConfigurationValue('CFG_CMP_SYSTEM_TITLE');
    MaLoad() // MarkAny
