...
import '~assets/css/THEME-4/theme-4.scss';
...
class AppInitializer {
  constructor() {
    ...
    Vue.config.performance = true; // 브라우저 devtool의 타임라인에서 컴포넌트 초기화, 컴파일,
    // 렌더링 및 패치 성능 추적을 활성화
  ...
  initPageLoader() {
    ...
    window.utils.install('openWindowPopup', (pageId, params = {}, title = pageId, options = {}) => {
      ...
      if (pageId) {
        url = `${_.replace(systemConfig.publicPath, /^\/$/, '')}${systemConfig.popupTheme4Url}#/${pageId}${queryString}`;
  ...
  createVueApp(VueApp, loginData) {
    ...
    Vue.mixin({
      ...
      // });
      // if (isChanged) {
      // utils.messageBox('confirm', '저장되지 않은 데이터가 있습니다. 이동하시겠습니까?', '',
      ...
      mounted() {
        ...
        if (componentName && componentName.length === 13 && componentName.split('PGE').length === 2) { // TODO
          // :
          // FIX
          // ME
