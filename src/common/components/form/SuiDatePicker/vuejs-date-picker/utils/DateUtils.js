...
const utils = {
  ...
  getWeekNum(value) {
    return moment(new Date(value.timestamp)).week();
  },

  getWeekNumber(week) {
    return this.getWeekNum(week[0]);
  },

  getWeekFromDate(week) {
    const weekNumber = this.getWeekNumber(week);
    return moment(week[0].timestamp).week(weekNumber).startOf('week');
  },

  getWeekToDate(week) {
    const weekNumber = this.getWeekNumber(week);
    return moment(week[0].timestamp).week(weekNumber).endOf('week');
  },

  getThisWeek() {
    const weekNumber = moment(new Date()).week();
    return {
      from: moment().week(weekNumber).startOf('week'),
      to: moment().week(weekNumber).endOf('week'),
    };
  },
}
