...
export default {
  ...
  methods: {
    ...
    editConnection(connection) {
      //dbl click rel
      if(localStorage.getItem('relSelected'))
      localStorage.removeItem('relSelected');    
      if (this.readonly) {
        return;
      }
    ...
    async handleChartMouseUp() {
      ...
      if (this.selectionInfo) {
        ...
        if (localStorage.getItem("relAttribute")) this.$emit("reldrop");
    ...
    guideLineTo(x1, y1, x2, y2) {
      ...
      lineTo(g, x1, y1, x2, y2, 1.1, "#a3a3a3", [5, 3]);
    },
      ...
      for (let position in connectorPosition) {
        ...
        connector
          .on("mousedown", function () {
            d3.event.stopPropagation();
            if (node.type === "Shut Down" || that.readonly) {
              utils.messageBox("alert", that.$t('MSG_ALT_NO_REL_FRM_SD'));
              return;
            }
            if (!localStorage.getItem("relAttribute")) {
              utils.messageBox("alert", that.$t('MSG_ALT_SLT_REL_TYPE'));
              return;
            }
              ...
              if (that.connectingInfo.source.id !== node.id) {
                ...
                let conn = {
                  source: {
                    ...
                    nodeId: that.connectingInfo.source.nodeId,
                    dataServiceId: that.connectingInfo.source.dataServiceId,
                  },
                  destination: {
                    id: node.id,
                    position: position,
                    relationNodeId: node.nodeId,
                  },
                  id: tempId,
                  type: "pass",
                  name: "Pass",
                ...
                that.setNodeInfoForRelation = conn;
                that.$emit(
                  "connect",
                  conn,
                  that.internalNodes,
                ...
                that.$emit("setNodeInfoForRelation", that.setNodeInfoForRelation);
                that.setNodeInfoForRelation = {};
              }
    ...
    removeNode(node) {
      ...
      utils.messageBox("confirm", this.$t("MSG_ALT_DLT_REL_DLT_ND"), null, () => {
        http.request(this.pageId, "DTS_RWR_00033", {
          data: dataParams,
          path: {
