<template>
  <div class="sui-input-label" :class="classes" :style="widthStyle">
    <div>
      <span>
       <label v-if="mask" ref="label"  :title="tooltip? tooltip:realText" :class="labelClasses"> {{ realText }}</label>
        <label v-else ref="label" :title="tooltip? tooltip:label" :class="labelClasses"> {{ label }}</label>
      </span>
