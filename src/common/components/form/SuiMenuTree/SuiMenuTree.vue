...
<script>
    import TreeItem from '../SuiTreeItem/SuiTreeItem.vue'
    ...
    export default {
        ...
        methods: {
            ...
            onItemDragStart(e, oriNode, oriItem) {
                ...
                e.dataTransfer.setData('text', '') // IE11 Fix
            ...
            onItemDrop(e, oriNode, oriItem) {
                ...
                if (this.draggedItem) {
                    ...
                    if (oriItem.folderYn === 'Y') {
                      ...
                      draggedItem.item.parentId = oriItem.id;
                      this.$nextTick(() => {
                      ...
                      // var draggedItem = this.draggedItem;
                      // draggedItem.item.parentId = oriItem.parentId;
                      // draggedItem.parentItem.splice(draggedItem.index, 1);
                      // oriItem.addAfter(draggedItem.item, oriNode);

                      var draggedItem = this.draggedItem;
                      if (draggedItem.item.parentId === oriItem.parentId) {
                        if (oriItem.order > draggedItem.item.order) {
                          draggedItem.parentItem.splice(draggedItem.index, 1);
                          oriItem.addAfter(draggedItem.item, oriNode);
                        } else {
                          draggedItem.parentItem.splice(draggedItem.index, 1);
                          oriItem.addBefore(draggedItem.item, oriNode);
                        }
                      } else {
                        draggedItem.item.parentId = oriItem.parentId;
                        draggedItem.parentItem.splice(draggedItem.index, 1);
