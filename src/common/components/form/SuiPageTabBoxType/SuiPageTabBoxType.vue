  ...
  <ul ref="scrollContainer" class="sui-tab-box-type__header">
    <div class="sui-tab-box-type__header__wrap">
      <div ref="scrollContent" class='sui-tab-box-type__header__container' :style="translateXStyle">
         ...
         <li v-for="(item,idx) in tabItems" v-show="item.ShowTab===false?false:true" :key="idx" @click="changeIdx(idx)" :class="{'sui-tab-box-type--active':currentTabIndex == idx, 'sui-tab-box-type--checked':item.checked}" :title="item.tabTitle" class="sui-tab-box-type__header-item">
  ...
  methods: {
    ...
    refresh(force = false) {
      if (force) {
        ...
        this.$nextTick(() =>{
          if(this.$refs.scrollContainer.getBoundingClientRect().width<1){
