<template>
  <div :class="classes" role="tree" onselectstart="return false">
    <span v-show="(itemCount-itemInvalidCount) < 1"  :class="(itemCount-itemInvalidCount) < 1? 'empty_txt': 'containerClasses'" role="group">
      No data to display
    </span>
    <ul v-show="(itemCount-itemInvalidCount) > 0" :class="containerClasses" role="group">
      <tree-item v-for="(child, index) in data"
                 ...
                 :leaf-selectable="leafSelectable"
 ...
 * 최종 수정 일시 : 2018 - 09 - 10
 */
import treeItem from './SuiTreeViewItem.vue';
...
export default {
  ...
  props: {
    ...
    leafSelectable: {type: Boolean, default: false},
  },
  data () {
    return {
      draggedItem: null,
  ...
  methods: {
    ...
    initializeDataItem (item) {
      ...
      if (item.isInvalid) this.itemInvalidCount = this.itemInvalidCount + 1;
