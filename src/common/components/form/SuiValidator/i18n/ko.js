...
const messages = {
  ...
  in: (field) => `${field} 항목의 값은 유효한 값이어야 합니다.`,
  ...
  not_in: (field) => `${field}항목은 유효한 값이어야 합니다.`,
  ...
  sui_required: (field) => field + ' 항목은 필수 정보입니다.',
  sui_custom: (field, params) => {
    if (params && params[1]) {
      return params[1]
    } else {
      return `${field}항목은 형식에 맞지 않습니다.`
    }
  },
  // sui_required: (field) => field + ' example 입니다.',
}
