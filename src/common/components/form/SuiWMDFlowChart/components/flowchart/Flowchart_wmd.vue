...
import { line2, lineTo } from "../../../SuiFlowChart/utils/svg";
...
export default {
  ...
  props: {
    ...
    nodes: {
      type: Array,
      default: () => [
        { id: 1, x: 640, y: 270, name: "Start", type: "start" },
  ...
  data() {
    return {
      ...
      datasets: [
        {
          id: "dsProcessTaskStandardId",
          type: "local",
          data: "localdata0",
          datafields: [
            { type: "string", name: "paramId", map: "paramId" },
            { type: "string", name: "paramName", map: "paramName" },
            { type: "string", name: "dvoName", map: "dvoName" },
            {
              type: "string",
              name: "paramMappingValue",
              map: "paramMappingValue",
            },
            {
              type: "string",
              name: "workflowNodeParamUid",
              map: "workflowNodeParamUid",
            },
            { type: "string", name: "workflowNodeUid", map: "workflowNodeUid" },
            { type: "string", name: "workflowId", map: "workflowId" },
          ],
        },
        {
          id: "dsWorkflowNode",
          ...
          datafields: [
            { type: "string", name: "workflowNodeUid", map: "workflowNodeUid" },
            { type: "string", name: "workflowId", map: "workflowId" },
            { type: "string", name: "nodeId", map: "nodeId" },
            { type: "string", name: "nodeName", map: "nodeName" },
            { type: "string", name: "nodeTypeCode", map: "nodeTypeCode" },
            { type: "string", name: "nodeTypeName", map: "nodeTypeName" },
            { type: "string", name: "buttonName", map: "buttonName" },
            ...
            { type: "string", name: "dataServiceId", map: "dataServiceId" },
            { type: "string", name: "deleteYn", map: "deleteYn" },
            { type: "string", name: "nodeOrder", map: "nodeOrder" },
            { type: "string", name: "nodeXvalue", map: "nodeXvalue" },
            { type: "string", name: "nodeYvalue", map: "nodeYvalue" },

            {
              type: "string",
              name: "workflowNodeParamList",
              map: "workflowNodeParamList",
            },
          ],
        },
      ],
  ...
  methods: {
    ...
    add(node) {
      ...
      localStorage.setItem('lastnode', JSON.stringify(node));     
    },
    cancelNode(){
      this.internalNodes.push(node);      
    },
    ...
    async handleChartMouseUp() {    
          ...
          if (this.connectingInfo.source.id !== this.hoveredConnector.node.id) {
            ...
            let conn = {
              source: {
                ...
                nodeId: this.connectingInfo.source.nodeId,
                position: this.connectingInfo.sourcePosition,
                dataServiceId: this.connectingInfo.source.dataServiceId,
              },
              destination: {
                id: this.hoveredConnector.node.id,
                relationNodeId: this.hoveredConnector.node.nodeId,
            ...
            // this.internalConnections.nodeId = conn.source.id
            // this.internalConnections.relationNodeId = conn.destination.id
            // this.internalConnections.relationSourceLocValue = conn.source.position
            // this.internalConnections.relationDestLocValue = conn.destination.position
            if(localStorage.getItem("relAttribute")){
              conn.type = localStorage.getItem("relAttribute");
            }
            this.internalConnections.push(conn);
	          this.setNodeInfoForRelation = conn;
            this.$emit("setNodeInfoForRelation", this.setNodeInfoForRelation);
            this.$emit(
              "connect",
              conn,
              this.internalNodes,
      ...
      if (this.selectionInfo) {
        //when connecting two nodes show popup.
        if(localStorage.getItem("relAttribute"))
         this.$emit("reldrop");
      ...
      return new Promise(function (resolve) {
        that.$nextTick(function () {
          ...
          that.internalConnections.forEach((conn) => {           
            ...
            conn.type = conn.relationNodeTypeCode;
    ...
    renderNode(node, isSelected) {
      ...
      //g = that.append("g").html("<span>hiiiii</span>");
      ...
      let text = "Admin";
      g.append('span').text(function(){
        return text;
      })
      .attr("cx", node.nodeXvalue)
      .attr("cy", node.nodeYvalue)
      .style('fill', 'black')
      .style('cursor', 'move')
      .style('font-size', '20')
      .style('z-index', '9');
    ...
    removeNode(node) {
      ...
      node.deleteYn = "Y";
      this.updatedNodes.dataServiceId = node.dataServiceId;
      this.updatedNodes.deleteYn = node.deleteYn;
      this.updatedNodes.moduleName = node.moduleName;
      this.updatedNodes.nodeId = node.nodeId;
      this.updatedNodes.nodeOrder = node.nodeOrder;
      this.updatedNodes.nodeName = node.nodeName;
      this.updatedNodes.nodeTypeCode = node.nodeTypeCode;
      this.updatedNodes.nodeTypeName = node.nodeTypeName;
      this.updatedNodes.nodeXvalue = node.nodeXvalue;
      this.updatedNodes.nodeYvalue = node.nodeYvalue;
      this.updatedNodes.pageId = node.pageId;
      this.updatedNodes.subModuleName = node.subModuleName;
      this.updatedNodes.workflowId = node.workflowId;
      this.updatedNodes.workflowNodeUid = node.workflowNodeUid;
      const workflowNodeUid = node.workflowNodeUid;
      const dataParams = {
        updateWorkflowNodeList: [],
        insertWorkflowNodeList: [],
        deleteWorkflowNodeList: [this.updatedNodes],
        //  deleteWorkflowNodeList: this.dsWorkflowNode.getRemovedRows(),
      };
      // utils.messageBox("warning", this.$t("MSG_ALT_DEL_NODE"));
      http
        ...
        .then(() => {
          ...
          //  this.dsWorkflowNodeRelation.submit();
          //this.dsWorkflowNode.submit();
      
          //this.setWorkflowNodeData();

          utils.messageBox('confirm', "Deleting nodes will also delete Relations", null, () => {
            this.$emit("onloadrefresh", this.updatedNodes);
            this.deleteRelationOnNodeDelete(node);
          });


        });
    },
    deleteRelationOnNodeDelete(node){
      let deleteRelations = [];
      ...
      for (let connection of connections) {
        ...
        deleteRelations.push(connection);
        //this.$emit("deleteRelationOnDeleteNode", connection);
      }
      // this.internalNodes.splice(this.internalNodes.indexOf(node), 1);
      this.$emit("deleteRelationOnDeleteNode", deleteRelations, 'delete');
  ...
  mounted() {
    this.$root.$on('flowchart', () => {     
      this.internalConnections.splice(this.internalConnections.length-1, 1);             
        });
    this.removeRelationsOnCancel();
    let that = this;
    that.init();
  ...
  watch: {
    removeLastComponent:function(value){     
     this.internalNodes.splice(this.internalNodes.length-1, 1);     
    },  
    internalNodes: {
      immediate: true,
      deep: true,
