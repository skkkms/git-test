<template>
  <div class="container">
      ...
      <button  @click="handleDataInput">Refresh</button>-->
    <flowchart
      :nodes="tempnode"
...
import Flowchart from "../components/flowchart/Flowchart_wmd";
import * as d3 from "d3";
import { roundTo20 } from "../../SuiFlowChart/utils/math";
...
export default {
  ...
  data: function () {
    return {
      ...
      DataValue: true,
      // refreshValues: false,
      // convasRefresh: this.$store.state.convasRefresh,
      tempnode: [],
  ...
  methods: {
    ...
    handleDataInput(updatedNodes) {
      ...
      const workflowId = this.setWorkflowMonitoringList.workflowId;
      const taskStandardId = this.setWorkflowMonitoringList.taskStandardId;
      
      http
        .request(this.pageId, "DTS_RWR_00057", {
          path: {
        ...
        .then((res) => {
          ...
          this.nodeData.map((item) => {
            let occurrenceTime = "";
            if(item.occurrenceTime != null){
              occurrenceTime = item.occurrenceTime.slice(0, -6);
            }
            ...
            let obj = this.tempnode.push({
              ...
              taskStandardId: item.taskStandardId,
    ...
    render: function (g, node, isSelected) {
      ...
      let borderColor = isSelected ? "#666666" : "#000000";
       if (node.type === "Start") {
        ...
        // body text
        let text = node.approvers[0].name+"\n"+"("+node.nodeId+")";
      ...
      function wrap(text, width) {
        ...
        //let occTimeX = node.x + node.width / 2 + 20;
        let occTimeY = bodyTextY-5 + 50;
        let unY = bodyTextY-5 + 70;

        let occTimeText = '';
        let userNameText = '';
        if (node.event === "Node Processing Complete"){
          occTimeText = node.occurrenceTime;
          userNameText = node.userName;
          ...
          .text(function () {
            ...
            utils.openLayerPopup('PGE_CMZ_00045', null, {
              pageInitialData: {
                knoxId: node.knoxId,
                userId: node.userId,
                loginId: node.loginId,
                userName: node.userName,
                userGlobalName: node.userGlobalName,
                departmentName: node.departmentName,
                departmentEnglishName: node.departmentEnglishName,
                displayLanguageId: node.displayLanguageId
              },
              size: 'lg',
            });
          });

        }else if(node.event === "Forced Termination"){

          occTimeText = node.occurrenceTime;
          userNameText = node.event;
