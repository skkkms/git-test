...
import CellTextFieldView from './CellTextFieldView';

class CellTextField extends GridCellBase {
  constructor(row, column, value, parentElement, propsData, dataset, grid) {
    ...
    if (this.componentInstance) {
      // this.componentInstance.$on('focus', () => {
      //   this.select();
      // })
    }
  }
  ...
  getValueFromEvent(event) { // 이벤트로부터 실제 dataset을 변경할 value 를 반환
    return this.componentInstance.realText
  }
  ...
  startDatasetUpdate() {
    this.dataset.startSetDataPool();
  }

  stopDatasetUpdate() {
    this.dataset.stopSetDataPool();
  }
