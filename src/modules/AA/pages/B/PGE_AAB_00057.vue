        ...
        <template>
          <div class="modal-body">
            ...
            <div class="comm_btn_wrap bt1">
              <div class="left_box mt10">
                * 기준일자에 적용되어 있는 시설운영정보가 선택한 생성일자로 동일하게 생성됩니다.
              </div>
...
<script>
export default {
  name: 'PGE_AAB_00057', // eslint-disable-line vue/name-property-casing
  components: {
  },
  props: {
    pageInitialData: {
    ...
    getFaciltList() {
      ...
      http.request(this.contextPageId, serviceName, {
        ...
        if (res.data && res.data.length > 0) {
          ...
          utils.messageBox('warning', this.$t('조건에 맞는 시설이 없습니다.'))
          this.FACILT_LIST = []
        }
      }).catch(error => {
        console.log(error)
      ...
      this.$nextTick(() => {
        if (this.validated()) {
          utils.messageBox('confirm', this.$t('일괄생성을 진행 하시겠습니까?'), null, () => { // 일괄생성을 진행 하시겠습니까?
            ...
            const serviceName = 'DTS_AAB_00029'
            http.request(this.contextPageId, serviceName, {
              query: {},
              path: {},
              data,
              ...
              if (res.data.resultCode === '-1') { // -1 이면 서버 메시지를 그대로 표시한다.
                ...
                utils.messageBox('warning', this.$t('기준일자에 해당하는 일별시설 데이타가 존재하지 않습니다.'))
              } else {
                utils.messageBox('warning', this.$t('일별 시설 일괄생성에 성공하였습니다.'))
