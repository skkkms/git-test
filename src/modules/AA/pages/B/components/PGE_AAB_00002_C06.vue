    ...
    this.columns = [
      ...
      },
      {
        ...
        component: {
          ...
          methods: {
            onInput(v) {
              that.dataSet.setRowValue(this.row, 'noticeDescrt', v)
      ...
      },
      {
        ...
        component: {
          ...
          methods: {
            onInput(v) {
              that.dataSet.setRowValue(this.row, 'noticeDescrtEng', v)
              this.$nextTick(() => that.fetchData())
            },
          },
        },
  ...
  methods: {
    ...
    onAdd() {
      if (this.dataSet.dataSize > 9) {
        this.alert('유의사항은 최대 10개까지 추가할 수 있습니다.') // 유의사항 최대 10개 제한
        return
      }
      this.dataSet.addRow({
        noticeDescrt: '',
      ...
      if(this.dataSet.getViewData()) {
        this.dataSet.getViewData().forEach(element => {
          let val = {
            ...
            noticeDescrt: element.noticeDescrt,
            noticeDescrtEng: element.noticeDescrtEng,
          }
          value.push(val)
        })
      }
