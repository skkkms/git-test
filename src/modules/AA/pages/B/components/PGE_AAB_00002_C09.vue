...
export default {
  name: 'PGE_AAB_00002_C09', // eslint-disable-line vue/name-property-casing
  components: {
    ALERT,
  },
  props: {
    ...
    this.columns = [
      ...
      },
      {
        ...
        component: {
          ...
          methods: {
            onInput(v) {
              that.dataSet.setRowValue(this.row, 'animlDescrt', v)
      ...
      },
      {
        ...
        component: {
          ...
          methods: {
            onInput(v) {
              that.dataSet.setRowValue(this.row, 'animlDescrtEng', v)
              this.$nextTick(() => that.fetchData())
            },
          },
        },
      },
      {
        ...
        component: {
          ...
          methods: {
            onInput(v) {
              that.dataSet.setRowValue(this.row, 'animlImagUrl', v)
              this.$nextTick(() => that.fetchData())
            },
          },
        },
  ...
  methods: {
    ...
    onAdd() {
      if (this.dataSet.dataSize > 9) {
        this.alert('동물관리는 최대 10개까지 추가할 수 있습니다.') // 동물관리 최대 10개 제한
        return
      }
      this.dataSet.addRow({
        animlDescrt: '',
      ...
      if(this.dataSet.getViewData()) {
        this.dataSet.getViewData().forEach(element => {
          let val = {
            ...
            animlDescrt: element.animlDescrt,
            animlDescrtEng: element.animlDescrtEng,
            animlImagUrl: element.animlImagUrl,
          }
          value.push(val)
        })
      }
