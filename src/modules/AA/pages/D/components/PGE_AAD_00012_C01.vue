<template>
  <ur-form-box toggleable>
    ...
    >
      <div v-if="!editMode">
        {{ this.getBnftKindCdNm }}
      </div>
      <ur-radio-wrapper
        v-else
        v-model="inputData.bnftKindCd"
        sm
        :items="codes.COD_IA_BENEFIT_KIND"
        @input="inputBnftKindCd"
      />
    </ur-form-item>
    ...
    >
      <div v-if="!editMode">
        {{ inputData.targetDescrt }}
      </div>
      <ur-text-field
        v-else
        v-model="inputData.targetDescrt"
        style="width:100%; height:auto;"
        maxlength="100"
      />
    </ur-form-item>
    ...
    >
      ...
      <cmp-single-file-uploader
        v-else-if="inputData.bnftKindCd=='02'"
        ref="thumbImagUrl1"
        v-model="inputData.thumbImagUrl"
        :mode="uploaderMode"
        :preview-mode="previewMode"
...
export default {
  ...
  data() {
    return {
      // 공통코드
      codeKeyList: ['COD_IA_BENEFIT_KIND'],
      codes: { COD_IA_BENEFIT_KIND: [] },
      bnftKindCdOld: '',
    }
  },
  computed: {
    ...
    getBnftKindCdNm() {
      return CommonUtils.getCodeText(this.codes.COD_IA_BENEFIT_KIND, this.inputData.bnftKindCd)
    },
    previewMode() {
