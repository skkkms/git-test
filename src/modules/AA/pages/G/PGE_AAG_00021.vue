  ...
  <sui-page class="custom_page">
    ...
    <sui-page-contents>
      ...
      <ur-data-grid
        ...
        :headers="missionGrid.headers"
        :data-source="missionGrid.dataSource"
        :options="missionGrid.options"
        :item-size="missionGrid.itemSize"
        :data-set-options="{id: 'id'}"
        @grid:scroll-bottom="onBottomMissionGrid"
      >
        <template #left-info>
          <cmp-ur-data-grid-left-info
            :total-count="missionGrid.itemSize"
...
export default {
  name: 'PGE_AAG_00021', // eslint-disable-line vue/name-property-casing
  components: {
  },
  data() {
    return {
  ...
  methods: {
    ...
    initializeSearchParams() {
      ...
      if (!utils.isEmpty(searchParams)) {
        ...
        this.$refs.missionGrid.mx_pager.rowsPerPage = this.searchParams.rowsPerPage
      } else {
        this.onClickReset()
      }
    },
    initializeHeaders() {
      this.missionGrid.headers = [
        {
          ...
          click: (value, row) => {
            ...
            utils.goPage('PGE_AAG_00023', { params })
          },
        },
        {
    ...
    onBottomMissionGrid() {
      const totalSize = this.missionGrid.itemSize
      const currentSize = this.missionGrid.dataSource.totalSize
      if (totalSize > currentSize) {
        this.selectMissionList()
      }
    },
    onClickReset() {
      this.searchParams = {
    ...
    onClickSearch() {
      ...
      this.$refs.missionGrid.setScrollLeft(0)
      this.missionGrid.dataSource.setData()
    ...
    onClickDelete() {
      const selectedRow = this.$refs.missionGrid.getSelected()
      ...
      utils.messageBox(
        ...
        null,
        () => this.deleteMission(eventIds),
      )
    },
    selectMissionList() {
      http.request(this.$options.name, 'DTS_AAG_00021', {
        query: {
          ...
          offset: this.missionGrid.dataSource.totalSize,
        },
      }).then(res => {
        this.missionGrid.dataSource.appendData(res.data.missionList)
        this.missionGrid.itemSize = res.data.missionListTotCnt
      })
    },
    deleteMission(eventIds) {
      http.request(this.$options.name, 'DTS_AAG_00025', {
        query: {
