  ...
  <sui-page class="custom_page">
    ...
    <sui-page-contents>
      <ur-form-box toggleable>
        ...
        >
          <ur-text-field
            v-model="searchParams.gameCdValue"
            width="100%"
            maxlength="50"
            @keyup.enter="onClickSearch"
            @keyup="searchParams.gameCdValue = adjustGameCdValue(searchParams.gameCdValue)"
          />
        </ur-form-item>
        <!-- 이벤트 명 -->
        <ur-form-item
      ...
      <ur-data-grid
        ...
        :headers="gameGrid.headers"
        :data-source="gameGrid.dataSource"
        :options="gameGrid.options"
        :item-size="gameGrid.itemSize"
        :data-set-options="{id: 'id'}"
        @grid:scroll-bottom="onBottomGameGrid"
      >
        <template #left-info>
          <cmp-ur-data-grid-left-info
            :total-count="gameGrid.itemSize"
...
export default {
  name: 'PGE_AAG_00041', // eslint-disable-line vue/name-property-casing
  components: {
  },
  data() {
    return {
  ...
  methods: {
    ...
    initializeSearchParams() {
      ...
      if (!utils.isEmpty(searchParams)) {
        ...
        this.$refs.gameGrid.mx_pager.rowsPerPage = this.searchParams.rowsPerPage
      } else {
        this.onClickReset()
      }
    },
    initializeHeaders() {
      this.gameGrid.headers = [
        {
        ...
        {
          text: this.$t('MSG_TXT_GAME_CD'), // 게임 코드
          value: 'gameCdValue',
          width: 120,
          minWidth: 120,
        },
        {
          ...
          click: (v, r) => {
            ...
            utils.goPage('PGE_AAG_00043', { params })
          },
        },
        {
    ...
    onBottomGameGrid() {
      const totalSize = this.gameGrid.itemSize
      const currentSize = this.gameGrid.dataSource.totalSize
    ...
    onClickSearch() {
      ...
      this.$refs.gameGrid.setScrollLeft(0)
      this.gameGrid.dataSource.setData()
    ...
    onClickDelete() {
      const selectedRow = this.$refs.gameGrid.getSelected()
    ...
    selectGameList() {
      http.request(this.$options.name, 'DTS_AAG_00041', {
        query: {
          eventPublctKindCd: this.params.eventPublctKindCd,
          gameCdValue: this.params.gameCdValue,
          ...
          offset: this.gameGrid.dataSource.totalSize,
        },
      }).then(res => {
        this.gameGrid.dataSource.appendData(res.data.gameList)
        this.gameGrid.itemSize = res.data.gameListTotCnt
      })
    },
    deleteGame(eventIds) {
      http.request(this.$options.name, 'DTS_AAG_00045', {
        query: {
