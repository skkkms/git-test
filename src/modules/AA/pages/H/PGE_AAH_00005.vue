...
export default {
  name: 'PGE_AAH_00005', // eslint-disable-line vue/name-property-casing
  ...
  created() {
    this.timeSettngValueArr[0] = ''
    this.timeCapaCntArr[0] = ''
  ...
  methods: {
    ...
    save() {
      ...
      http.request(this.$options.name, 'DTS_AAH_00012', {
        data: param,
      }).then(() => {
        utils.messageBox('success', this.$t('MSG_ALT_SAVE_DATA'))
        const slotStartDay = this.$route.params.slotStartDay
        const showDate = this.$route.params.showDate
        utils.goPage('PGE_AAH_00004', {
          slotStartDay: slotStartDay,
          showDate: showDate,
        })
      }).catch(error => {
        console.log(error)
