...
export default {
  name: 'PGE_AAI_00005', // eslint-disable-line vue/name-property-casing
  components: {
  },
  data() {
    return {
  ...
  methods: {
    ...
    save() {
      ...
      http.request(this.$options.name, 'DTS_AAI_00032', {
        data: param,
      }).then(res => {
        if (res.data.duplicatePk === 'true') {
          utils.messageBox("alert", this.$t('MSG_ALT_ALREADY_CALL_KIND'));
        } else {
