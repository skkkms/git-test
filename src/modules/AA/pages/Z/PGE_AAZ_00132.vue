...
export default {
  ...
  data() {
    return {
      ...
      codes: {
        COD_IA_COND_KIND: [], COD_IA_COND: [], COD_IA_COND_WEATHER: [], COD_IA_COND_FACILITY: [], COD_IA_COND_COUPON: [],
      },
  ...
  created() {
    ...
    http.mergeCodeList(this.$options.name, this.codeKeyList, this.codes).then(() => {
      this.columns = [
        {
          text: this.$t('MSG_TXT_NUMBER_ROW'),
          value: '__idx',
          width: 50,
          type: 'number',
          component: {
        ...
        // 항목
        {
          ...
          component: {
            ...
            template: `<div v-if="this.row.condKindCd=='01'?true:false">
                            <ur-text-field  v-model = "row.condNm" readonly/>
        ...
        // 선택값
        {
          ...
          component: {
            ...
            template: `
                        ...
                        <div v-else>
                          <div      v-if="this.row.condCd=='01'" ><ur-dropdown v-model="row.condValue" :items="COD_IA_COND_WEATHER" @input="onClick(row.condValue)"/></div>
            ...
            computed: {
              ...
              // 파크구분
              parkKindCd() {
                return that.pageInitialData.parkKindCd === '01'
              },
            },
            methods: {
              onClick(v) {
  ...
  mounted() {
    ...
    if (this.pageInitialData.inputType === 'I') {
      ...
      if (this.pageInitialData.initData !== null && this.pageInitialData.initData.length > 0) {
        this.$refs.grid.setData(this.pageInitialData.initData)
      } else {
    ...
    // 수정일 경우
    else if (this.pageInitialData.selectCondCdList.length > 0) {
      this.$refs.grid.setData(this.pageInitialData.selectCondCdList)
    }
  },
  methods: {
    ...
    // 그리드
    resetSelect() {
      const temp = this.$refs.grid.getDataSet()
    ...
    onClickSave() {
      ...
      if (this.inputList.data[0].condKindCd !== '01') {
    ...
    isValidation() {
      ...
      for (let i = 0; i < this.inputList.dataSize; i++) {
        this.inputList.data[i].__selected = false // 셀렉트 초기화

        // 같은 항목은 중복될 수 없음.
        for (let j = 0; j < this.inputList.dataSize; j++) {
          if (i !== j && this.inputList.data[i].condCd === this.inputList.data[j].condCd) {
            const msg = utils.strFormat(this.$t('MSG_ALT_ITEM_REDND')) // 항목은 중복 설정할 수 없습니다.(선택 값 : 확인)
            utils.messageBox('warning', msg)
            return false
          }
        }
    ...
    chgTxtCode() {
      for (let i = 0; i < this.inputList.data.length; i++) {
