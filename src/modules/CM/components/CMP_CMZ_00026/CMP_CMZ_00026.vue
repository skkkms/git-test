    ...
    <div class="receiver_list_wrap">
      ...
      >
        <ul class="disable-select">
          <li
            v-for="(itm,index) in pushRecipientList"
            :key="index"
...
export default {
  ...
  props: {
    ...
    recptDispFormat: {
      ...
      required: false,
      default() {
        return `\${userName} [\${email}]`; // eslint-disable-line
      },
    },
  ...
  computed: {
    recipientAllCnt() {
      return this.pushRecipientList.length;
    },
  },
  watch: {
    pushRecipientList: {
  ...
  methods: {
    ...
    addRecipient(recInfo) {
      ...
      if (!utils.isEmpty(utils.find(this.pushRecipientList, { receiptId: _recInfo.email }))) {
        utils.messageBox('alert', this.$t('MSG_BTN_CFRM'), this.$t('MSG_ALT_IMG_RGST_ID'));
        return false;
      }

      this.pushRecipientList.push(_recInfo);
      const _lastRecipient = utils.last(this.pushRecipientList);
      if ([1, 2, 3, '1', '2', '3'].indexOf(_lastRecipient.pushRecipientType) === -1) {
        _lastRecipient.pushRecipientType = this.pushRecipientType;
      }
     ...
     */
    changeRecipientType() {
      this.pushRecipientList.forEach(itm => {
        if (itm.selected) {
          itm.pushRecipientType = this.pushRecipientType;
        }
      });
      this.countRecipientTypes();
    },
    ...
    onClickRecipient(event, id) {
      ...
      if ((event.shiftKey && event.ctrlKey) || event.ctrlKey) {
        ...
        let _beginIndex = 0;
        for (let i = 0; i < this.pushRecipientList.length; i++) {
          if (this.pushRecipientList[i].id == id) {
            _beginIndex = i;
            break;
          }
        }
        ...
        for (let i = 0; i < _beginIndex; i++) {
          if (this.pushRecipientList[i].selected) {
            _endIndex = i;
          }
        }
        if (_endIndex == -1) {
          for (let i = _beginIndex + 1; i < this.pushRecipientList.length; i++) {
            if (this.pushRecipientList[i].selected) {
              _endIndex = i;
              break;
            }
        ...
        if (_endIndex == -1) {
          this.pushRecipientList[_beginIndex].selected = true;
        } else {
          this.pushRecipientList.forEach(itm => {
            itm.selected = false;
          });
          for (let i = 0; i <= Math.abs(_beginIndex - _endIndex); i++) {
            this.pushRecipientList[Math.min(_beginIndex, _endIndex) + i].selected = true;
          }
        }
      } else {
        this.pushRecipientList.forEach(itm => {
    ...
    moveTop() {
      ...
      this.pushRecipientList.forEach((itm, index) => {
        if (itm.selected) {
          _selectedIndices.push(index);
        }
      });
      const _removedRows = utils.pullAt(this.pushRecipientList, _selectedIndices);
      for (let i = _removedRows.length - 1; i >= 0; i--) {
        this.pushRecipientList.unshift(_removedRows[i]);
      }
    },
    moveUp() {
      const _selectedIds = [];
    ...
    moveUpSingle(id) {
      ...
      for (let i = 0; i < this.pushRecipientList.length; i++) {
        if (id == this.pushRecipientList[i].id) {
          _selectedIndex = i;
          break;
        }
      }
      ...
      const _removedRow = utils.pullAt(this.pushRecipientList, _selectedIndex);
      this.pushRecipientList.splice(_selectedIndex - 1, 0, _removedRow[0]);
      return true;
    },
    moveDownSingle(id) {
      let _selectedIndex = 0;
      ...
      if (_selectedIndex == this.pushRecipientList.length - 1) {
        return false;
      }
    ...
    moveBottom() {
      ...
      for (let i = 0; i < _removedRows.length; i++) {
        this.pushRecipientList.push(_removedRows[i]);
      }
    },
    deleteSelection() {
      for (let i = this.pushRecipientList.length - 1; i >= 0; i--) {
        if (this.pushRecipientList[i].selected) {
          if (utils.isEqual(this.pushRecipientList[i].disFlag, 'true') === false) {
            this.pushRecipientList.splice(i, 1);
          }
        }
      }
    },
     ...
     */
    getRecptList() {
      return utils.cloneDeep(this.pushRecipientList);
    },
  },
};
</script>
