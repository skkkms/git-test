<template>
  <div>
    ...
    <div class="comm_scr_wrap">
      <div class="row">
        <div class="col-xs-1">
          <sui-input-label :label="$t('MSG_TXT_MDLE')" :required="false" :vertical="false" />
        </div>
        <div class="col-xs-11">
    ...
    <div class="comm_gridTop_wrap">
      <div class="btn_wrapL" style="height:24px;">
        <div class="totalList" v-show="selectedPageCount === 0">{{ totalCntPage }}</div>
        <sui-checkbox v-show="selectedPageCount" :checked="true" :disabled="false" class="num">{{ selectedCountMsg }}</sui-checkbox>
      </div>
    </div>
    <table class="table table-fixed">
      ...
      <tr>
        ...
        <th style="width:100px"> {{ $t('MSG_TXT_MDLE') }} </th>
        <th style="width:100px"> {{ $t('MSG_TXT_SMDLE') }} </th>
        <th style="width:150px"> {{ $t('MSG_TXT_PAGE_ID') }} </th>
        <th class="col-xs"> {{ $t('MSG_TXT_PAGE_NM') }} </th>
      </tr>
      </thead>
      <tbody>
      <draggable
        ...
        @end="onDragEnd($event)"
      >
        <tr v-for="(item, index) in localdata0" :key="item.pageId" :class="{ dragitem: item.check }">
          <td style="width:30px;">
            <sui-checkbox
              :checked="item.check"
              :value="item.check"
          ...
          <td style="width:100px"> {{ item.moduleName }} </td>
          <td style="width:100px"> {{ item.subModuleName }} </td>
          <td style="width:150px"> {{ item.pageId }} </td>
          <td class="col-xs">{{ item.pageName }} </td>
        </tr>
      </draggable>
      </tbody>
    </table>
...
export default {
  name: 'PGE_CMP_00011_T03', // eslint-disable-line vue/name-property-casing
  components: {
    draggable,
  },
  extends: BasePage,
  ...
  methods: {
    onCheckBoxClick(idx) {
      ...
      this.$emit('itemChecked', this.localdata0);
    },
    onPageSearchClick() {
