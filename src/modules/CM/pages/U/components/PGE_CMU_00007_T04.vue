...
export default {
  name: 'PGE_CMU_00007_T04', // eslint-disable-line vue/name-property-casing
  extends: BasePage,
  props: {
    pageId: {
      type: String,
  ...
  methods: {
    selectSubComponent() {
      // 선택한 컴포넌트 등록 서브컴포넌트 목록 조회
      http.request(this.pageId, 'DTS_CMU_00048', {
        path: {
    ...
    onNewClick() {
      if (!this.componentId) {
        ...
        utils.openLayerPopup('PGE_CMU_00008', this.onPopupClose, {
    ...
    addRows(list) {
      const addNewRows = [];
      const componentList = this.ds0.getRawData();
      utils.forEach(list, data => {
        const { componentId } = data;
        if (utils.findIndex(componentList, { componentId }) === -1) {
    ...
    for(var i=0;i<addNewRows.length;i++){
      const newAddedRow = {
         componentTypeName: addNewRows[i].componentTypeName,
         moduleName: addNewRows[i].moduleName,
         subModuleName: addNewRows[i].subModuleName,
         componentId: addNewRows[i].componentId,
         componentName: addNewRows[i].componentName,
         componentTag: addNewRows[i].componentTag,
       };
    ...
    onSaveClick() {
      if (!this.componentId) {
        ...
        utils.messageBox('confirm', this.$t('MSG_ALT_WANT_SAVE'), null, () => {
          const subComponentList = this.ds0.getRawData();
          const dataParams = [];
          ...
          http.request(this.pageId, 'DTS_CMU_00049', {
            path: {
              'component-id': this.componentId,
            },
            data: dataParams,
