<template>
  <div>
    ...
    >
        ...
        <div class="row">
          ...
          <div class="col-xs-5">
            <sui-text-field
              ...
              :placeholder="$t('MSG_ALT_INP_CMPNT_ID_NM')"
              maxlength="100"
      ...
      <div class="mrB30" />
      <div class="comm_shut_wrap" style="justify-content: left">
        <div class="inbox">
          <sui-s-grid
            ref="grdOrgList"
...
export default {
  ...
  methods: {
    onSearchMappedPage() {
      http.request(this.pageId, 'DTS_CMU_00045', {
        path: {
    ...
    onSearch() {
      ...
      const queryParams = {
        ...
        componentTypeCode: utils.trim(this.frame0.selectData.dataServiceTypeCode.key),
        componentSearchWord: utils.trim(this.frame0.selectData.searchText),
      };

      http.request(this.pageId, 'DTS_CMU_00026', {
        query: queryParams,
      }).then(res => {
        if (res.data) {
          const orgData = [];
          utils.forEach(res.data, obj => {
            const objData = {
              componentTypeName: utils.trim(obj.componentTypeName),
              moduleName: utils.trim(obj.moduleName),
              subModuleName: utils.trim(obj.subModuleName),
              componentId: utils.trim(obj.componentId),
              componentName: utils.trim(obj.componentName),
              componentTag: utils.trim(obj.componentTag),
            };
    ...
    onSave() {
      ...
      utils.messageBox('confirm', this.$t('MSG_ALT_WANT_SAVE'), null,
        (/* ok */) => {
          ...
          http.request(this.pageId, 'DTS_CMU_00046', { // dataservices reg.
            path: {
              'page-id': this.dataPageId,
            },
            data: dataParams,
