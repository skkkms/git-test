...
export default {
  name: 'PGE_CMZ_00001_P01', // eslint-disable-line vue/name-property-casing
  extends: BasePage,
  props: {
    pageId: {
      type: String,
  ...
  methods: {
    ...
    selectMaster() {
      ...
      // 수정화면 (코드 조회)
      http.request(this.pageId, 'DTS_CMZ_00012', {
        path: {
    ...
    checkCodeId() {
      ...
      http.request(this.pageId, 'DTS_CMZ_00004', {
        query: {
    ...
    onClickBtnSave() {
      // 변수범위가 다름 다른방법 확인중
      let runService = 'DTS_CMZ_00009'; // 등록
      if (this.isModification) {
        runService = 'DTS_CMZ_00011'; // 수정
      }
